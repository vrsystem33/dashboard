...
@Component({
  selector: 'app-carrier-categories',
  standalone: true,
  imports: [
    CommonModule,
    ToastModule,
  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 class="text-2xl font-semibold">Categorias de Transportadoras</h1>
          <p class="text-muted-color">Organize e mantenha as categorias dos seus transportadoras</p>
        </div>
      </div>
...
export class CarrierCategoriesPage implements OnInit {
  ...
  constructor(
    private service: CarrierCategoriesService,
  ...
  openEdit(category: CarrierCategory) {
  ...
  save(payload: CarrierCategoryCreateRequestDto | CarrierCategoryUpdateRequestDto) {
    ...
    if (isEdit && target?.id) {
      ...
      this.service.create(payload as CarrierCategoryCreateRequestDto).pipe(take(1)).subscribe(() => {
  ...
  confirmDelete(category: CarrierCategory) {
