...
import {
  CarrierCategoriesService,
  CarrierCategory,
  CarrierCategoryCreateRequestDto,
  CarrierCategoryUpdateRequestDto
} from '../carrier-categories.service';
...
@Component({
  ...
  template: `
    <p-dialog
      ...
      [resizable]="false"
      [closable]="true"
      [header]="carrier ? 'Editar Transportadora' : 'Novo Transportadora'"
      (onHide)="cancel.emit()"
    >
      <ng-template #content>
...
export class CarrierDialogComponent implements OnChanges {
  @Input() visible = false;
  @Input() carrier: CarrierRow | null = null;
  @Input() categories: CarrierCategory[] = [];
  @Output() save = new EventEmitter<CarrierCreateRequestDto | CarrierUpdateRequestDto>();
    ...
    if (changes['carrier']) {
      if (this.carrier) {
        this.form.reset({
          ...
          postal_code: this.carrier.postal_code ?? '',
          address: this.carrier.address ?? '',
          number: this.carrier.number ?? '',
          neighborhood: this.carrier.neighborhood ?? '',
          complement: this.carrier.complement ?? '',
          city: this.carrier.city ?? '',
  ...
  createCategory(payload: CarrierCategoryCreateRequestDto | CarrierCategoryUpdateRequestDto) {
    this.categoriesService.create(payload as CarrierCategoryCreateRequestDto).pipe(take(1)).subscribe({
      next: (category) => {
  ...
  private buildPayload(): CarrierCreateRequestDto | CarrierUpdateRequestDto {
    ...
    return cleaned as CarrierCreateRequestDto | CarrierUpdateRequestDto;
  }
}
