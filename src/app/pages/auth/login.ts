...
import { selectAuthError, selectAuthStatus } from '@app/core/auth/auth.selectors';
import { loginRequested } from '@app/core/auth/auth.actions';
                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        ...
                        <div>
                            ...
                            <input pInputText id="email1" type="text" placeholder="Email address or Username" class="w-full md:w-120 mb-8" [(ngModel)]="email" />
                            ...
                            <p-button
                                styleClass="w-full mb-4 flex items-center justify-center gap-2"
                                (onClick)="signIn()"
                                [disabled]="(status$ | async) === 'authenticating'">

                                <ng-container *ngIf="(status$ | async) === 'authenticating'; else labelSignIn">
                                    <p-progress-spinner
                                        strokeWidth="8"
                                        fill="transparent"
                                        ...
                                        *ngIf="(status$ | async) === 'authenticating'"
                                    />
                                </ng-container>

                                <ng-template #labelSignIn>
                                    Sign In
                                </ng-template>
                            </p-button>

                            <p-message *ngIf="messageError" severity="error" [text]="messageError"></p-message>
                            <p-message *ngIf="(error$ | async) as err" severity="error" [text]="err"></p-message>
                        </div>
                    </div>
                </div>
            </div>
